...
namespace NBidi
{
    ...
    public class NBidi
    {
        ...
        public static Func<CharData[], List<List<CharData>>> LineWrapperFunc
        {
            get;
            set;
        }
        ...
        public class Paragraph
        {
            ...
            public CharData[] TextData
            {
                get { return _text_data; }
            }
            ...
            /// </summary>
            private void ReorderString(int start, int end)
            {
                ...
                for (byte el = highest; el >= lowest_odd; --el)
                {
                    for (int i = start; i < end; ++i)
                    {
                        if (_text_data[i]._el >= el)
                        {
                            ...
                            int l2_start = i;
                            int limit = i + 1;
                            while (limit < end && _text_data[limit]._el >= el)
                            {
                                ++limit;
                            }
